...
if check_password():
    ...
    try:
        ...
        with tabs[0]:
            ...
            with c1:
                d_val = st.date_input("Dzień:", value=datetime.now(), key="d_in")
            with c2:
                st.write("###")
                all_d = st.checkbox("Wszystkie dni", value=False, key="a_in")
            with c3:
                search_in = st.text_input("🔍 Szukaj:", key="s_in")
        ...
        with tabs[2]:
            ...
            if not df_empty.empty:
                ...
                ed_empty = st.data_editor(df_empty_grouped[['Przewoźnik', 'Auto', 'Kierowca', 'STATUS']], use_container_width=True, key="ed_empty", 
        ...
        with tabs[3]:
            ...
            with st.form("form_empties"):
                ...
                with c3: 
                    f_c = st.selectbox("PRZEWOŹNIK", sorted(df_puste['Przewoźnik'].unique()) if not df_puste.empty else ["Brak"])
                ...
                if st.form_submit_button("DODAJ SLOT"):
                    match = df_puste[df_puste['Przewoźnik'] == f_c].iloc[0]
                    new_row = {"Data": str(f_d), "Nr Slotu": f_s, "Godzina": f_g, "Hala": f_h, "Przewoźnik": f_c, "Auto": match['Auto'], "Kierowca": match['Kierowca'], "STATUS": f_st, "Nr Proj.": "EMPTIES"}
            ...
            ed_sl = st.data_editor(df_sl[['Data', 'Nr Slotu', 'Godzina', 'Hala', 'Przewoźnik', 'Auto', 'Kierowca', 'STATUS', 'NOTATKA']], use_container_width=True, key="ed_sl", column_config=column_cfg_main, hide_index=True)
